...
```
console.log() is a function used to print information to the console. return is a call to pass some value back up to where the call was made.
```

**_What's the difference between defining and calling a function?_**

```
Defining a function means writing a function with all of the code inside it, but simply defining a function does not run any of the code inside that function. It only lays out to the computer what that function means. To run that code, you would need to "call" the function
```
...
```
A string can be any text inside double or single quote. For example, "Hello, World!"
```

**_Number_**

```
Positive or negative integers that can be used to perform mathematical operations in JavaScrpit.
```

**_Boolean_**

```
Used to execute code based on conditional logic with just two possible values: true and false
```

**_Array_**

```
A type of object used to store data, consisting of an ordered collection or list containing any amount of data types. it appears like: [item 1, item 2, etc]
```
